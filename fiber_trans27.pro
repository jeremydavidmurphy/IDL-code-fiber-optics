PRO fiber_trans27, fitslist, APPTYPE=apptype, DARKSWITCH=darkswitch

; THIS ROUTINE IS JUST A SLIGHT MODIFICATION TO REDUCE THE VP2.7 DATA.
...
plottingscale = 'on' ;turn this on if you want to see a plot showing the fiber centroid spacing (i.e. the radial distance between every fiber). this also plots the rotation correction plot.
...
nf = 246 ;The number of fibers that should exist in the frame. IF THERE ARE DEAD FIBERS, DO NOT REDUCE THIS NUMBER! Just put their # into the 'deadfibers' array above.
...
; ellipse-aperture photometry routine. (R1 is the radial distance from fiber #1)
   ...
   if (DARKSWITCH eq 'on') then begin
       ...
       dark = float(dark)
       image = image - dark
       if (max(image[100:200,100:200]) gt 30000.0) then begin
          image = image - 32768.0 ;a fudge to capture some funny-busniess which depends on the system you're running this on
   ...
   nameout6 = tempname[0]+'_IFUscale.txt'
   nameout7 = tempname[0]+'_IFUcen.txt'
...
;      line = 'circle('+strn(fiberarray[0,j])+','+strn(fiberarray[1,j])+',32)' ;use for the large lens imaging set up
      line = 'circle('+strn(fiberarray[0,j])+','+strn(fiberarray[1,j])+',24) # text={'+strn(j+1)+'}' ;this line adds the fiber numbers to each circle.
      printf,55,line
...
;average radial spacing between all the fibers in the image. the
;second bit calculates the rotation.
...
; the first is the old version that compared all neighboring fibers.
;         if (dx[k] lt 90 and dx[k] gt 0 and abs(dy[k]) lt 30) then begin
;            arc = atan(dy[k],dx[k]) ;the angle, in radians, between two fibers
;            angle = [angle,arc]
;         endif
; this new one compares fibers that have a minimum separation of > 5
; fibers (i.e. ~1/2 of the IFU head).
         if (dx[k] gt 450 and abs(dy[k]) lt 30) then begin
            arc = atan(dy[k],dx[k]) ;the angle, in radians, between two fibers
            angle = [angle,arc]
         endif
      endfor
   ...
   radius = radius[ikeep]
   angleR = angle
   angleD = angle[1:*] * (180.0/!pi) ;the angle, in degrees
   ...
   sd = sqrt(m[1])
   m = [m,sd]
   scale = 340.0 / m[0] ;the VP2.7 fiber pitch is 340 micron center-to-center spacing
...
; the center is now estimated from the position of all fibers,
...
;   Ryreal = yreal * cos(rotateR) - xreal * sin(rotateR)
  ...
  if (plottingscale eq 'on') then begin
      window,0,retain=2
      loadct,0,/silent
      plot,radius*scale,/ynozero,ytitle='Fiber Spacing (microns)',psym=1
      oplot,[0,n_elements(radius)],[m[0]*scale,m[0]*scale]
;      xyouts,10,(m[0]*scale)+(0.1*m[0]*scale),'The median of the fiber spacing...',charsize=2
      if (pauseplot eq 'on') then pause else wait,3.0
      plot,angleR,ytitle='Angle of rotation (radians)',psym=1
      oplot,[0,n_elements(angleR)],[rotateR,rotateR]
;      xyouts,10,rotateR+(0.1*rotateR),'The median of the angles...',charsize=2
      if (pauseplot eq 'on') then pause else wait,3.0
      
      window,0,retain=2,xsize=700,ysize=700
      loadct,4
      plot,offsetx,offsety,psym=2,/iso
      oplot,[-3000,3000],[0,0]
      oplot,[0,0],[-3000,3000]
      if (pauseplot eq 'on') then pause else wait,1.5
      oplot,Roffsetx,Roffsety,psym=4,color=150
      if (pauseplot eq 'on') then pause else wait,3.0
   endif
...
; the data is written out to a text file
   free_lun,5
   openw,5,nameout6
   printf,5,'The calculated rotation, in radians, is '+strn(rotateR)+'.'
   ...
   printf,5,strn(m)
   printf,5,''
   printf,5,'The fiber offsets, after rotation, in microns, from the center of the IFU:'
   form0 = '(a5,3x,a10,2x,a10,3x,a8)'
   printf,5,['Fib #','X_off (um)','Y_off (um)','Rad (um)'],format=form0
   form1 = '(i3,5x,f10.4,2x,f10.4,2x,f10.4)'
   for k=0,nf-1 do begin
      if (counter[all] eq 0) then piece = [k+1,Roffsetx[k],Roffsety[k],radoff[k]]
      if (counter[all] eq 1) then piece = [k+nf+1,Roffsetx[k],Roffsety[k],radoff[k]]
      printf,5,piece,format=form1
   endfor
   free_lun,5

; the IFUcen.txt file is created. Based on the IFUcen.txt scale
; (0.0210088) converts microns to arcseconds
   free_lun,5
   openw,5,nameout7
   micronstoarcsec = 7.14301/340.0
   Soffsetx = micronstoarcsec * Roffsetx
   Soffsety = micronstoarcsec * Roffsety
   form1 = '(i3,5x,f10.4,2x,f10.4)'
   for k=0,nf-1 do begin
      if (counter[all] eq 0) then piece = [k+1,Soffsetx[k],Soffsety[k]]
      if (counter[all] eq 1) then piece = [k+nf+1,Soffsetx[k],Soffsety[k]]
      printf,5,piece,format=form1
   endfor
   free_lun,5
