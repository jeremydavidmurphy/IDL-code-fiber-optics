PRO eeL, frameL, darkL, srad=SRad, SS=stepsize, EE=eee

; This code is a reworking of ee1, which plots the EE v radius for an
; individual fiber. It now reads in a list of both fiber frames and
; their corresponding darks.
...
for j=0,n0-1 do begin ;a loop through each fiber image
   ...
   names[j] = t[0]
    ...
    dark = dark * adj
    print,'The BG is being shifted by '+strn(t1m)
    image = image - dark
    ...
    loadct,0
    xyouts,0.05,0.05,names[j],/normal,charsize=1.5
    loadct,37
...
repeat begin
    if (done eq 'y') then begin
        print,'Enter a lower X:'
        read,xdn
        print,'Enter an upper X:'
        read,xup
    endif
    for j=0,n0-1 do begin
        ...
        if (j eq 0) then begin
            ...
            plot,radius,eep,xtitle='Radius (pixels)',$
              ...
              xrange=[xdn,xup],/xstyle,/ynozero
        endif
        loadct,4
        oplot,radius,eep,color=colors[j]
        xyouts,0.75,0.2+(j*0.04),names[j],/normal,charsize=1.2,color=colors[j]
    endfor
    print,'Change the X and Y axis ranges (Y/N)?'
    read,done
endrep until (done eq 'n')

print,'Save a hardcopy (Y/N)?'
read,ans
if (ans eq 'y') then begin
    set_plot,'ps'
    device,file='eeL.ps',/color
    for j=0,n0-1 do begin
        radius = data[0:counts[j],0,j]
        eep = data[0:counts[j],1,j]
        if (j eq 0) then begin
            loadct,0
            plot,radius,eep,xtitle='Radius (pixels)',/ynozero,$
              ytitle='Encircled Energy',charsize=1.0,/nodata,$
              xrange=[xdn,xup],/xstyle,xthick=3,ythick=3,charthick=3
        endif
        loadct,4
        oplot,radius,eep,color=colors[j],thick=3
        xyouts,0.75,0.2+(j*0.03),names[j],/normal,charsize=1.0,$
          color=colors[j],charthick=3
    endfor
    device,/close_file
endif
