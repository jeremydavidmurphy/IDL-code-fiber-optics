PRO fratio_v7, EE=ee, RADIUS=starting_radius, STEP=stepsize

;This code now accepts a list of frames, named 'fratio.list'.
;The list should have the biggest spot listed first

ans=''
titl=''
wave = [3370, 3500, 3650, 3800, 4000, 4200, 4500, 5000, 5500, 6000]
...
IF (N_Elements(ee) EQ 0) THEN ee=0.99 

readcol,'fratio.list',files,FORMAT='A'
files = transpose(files)
num = n_elements(files)

images = fltarr(512,512,num)
n = n_elements(images[*,*,0])
std_images = dblarr(n,num)
x_axis = Indgen(n,/long)
t1=dblarr(4,num) ;The total chip totals
t2=dblarr(num) ;The totals enclosed in the last iteration of the initial pixel cut
t3 = t2 ;The chip totals
ellipse = fltarr(2,120)
cut=''
d=double(1.27);Change this if your stepsize changes
center = fltarr(2,num)
diameter = dblarr(num)
radius = diameter
dd = dblarr(60)
RA = dblarr(512,512,num)
ax = fltarr(2) & sax = ax
width=intarr(num)
ttls=dblarr(num) ;The totals WITHIN THE ROI (which is the new box)
data = dblarr(10000,2,num)
...
FOR k=0,num-1 DO BEGIN 
    ...
    im = images[*,*,k]
    std_images[*,k] = im[bsort(im)]
    t1[0,k] = total(im[1:10,1:10])/100.

    t1[1,k] = total(im[501:510,1:10])/100.
    t1[2,k] = total(im[1:10,501:510])/100.
    t1[3,k] = total(im[501:510,501:510])/100.
ENDFOR

print,'Samples of the corner totals:'
print,t1
pause
...
FOR k=0,num-1 DO BEGIN
    ...
    cut = 0.07 * (total(std_images[n-10:n-1,k])/10.0)
    index = Where(oneimage GE cut)
    t2[k] = total(oneimage(index))
    t3[k] = total(oneimage)
    percent = t2[k]/total(oneimage)
    ellipse[*,*] = Fit_Ellipse(index, XSize=512, YSize=512, CENTER=c1, AXES=ax)
    ...
    xgraph = total(oneimage[*,center[1,k]-5:center[1,k]+5],2)/11
    ygraph = total(oneimage[center[0,k]-5:center[0,k]+5,*],1)/11
    ...
    plot,xgraph,title='X cross section',ytitle='Counts (ADU)',xtitle='The radius is :'+strn(radius[k]),$
      subtitle='The EE is :'+strn(percent),xrange=[0,512],xstyle=1,yrange=[0,max(ygraph)],charsize=1.3
    loadct,4
    ...
    x2 = indgen(512-center[0,k])+1
    x1 = reverse(indgen(center[0,k]))
    xline = [x1,x2]
    y2 = indgen(512-center[1,k])+1
    y1 = reverse(indgen(center[1,k]))
    yline = transpose([y1,y2])
    Xarray = fltarr(512,512) & Yarray = Xarray
    
    FOR i=0,511 DO Xarray(*,i) = xline
    FOR i=0,511 DO Yarray(i,*) = yline
...
; OF THE ROI'S
w2 = min([center[0,0],511-center[0,0],center[1,0],511-center[1,0]])
width[0] = 2*w2
ix1 = center[0,0]-w2
ix2 = center[0,0]+w2
iy1 = center[1,0]-w2
