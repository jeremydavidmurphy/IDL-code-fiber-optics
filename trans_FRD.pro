...
outname = 'vp6t_before' ;the output name that get's subscripted. The final
...
psize = 0.000012     ; the FLI camera pixel size, in meters. (change to 0.000024
                     ; for binned data)
thresh = 3000.0      ; the threshold above which the code fits an ellipse for 
...
starting_radius = 10 ; This sets the beginning radius for the FRD tests.
                     ...
                     ; good COGs.
eenergy = 0.99       ; How far the fratioF routine integrates up to.
                     ; Don't change this as fratioF is expecting this number...
        ...
        for k=0,ntemp-1 do begin
            ...
            repeat begin ;the exp. time is picked out of the header
                ...
                if (hh[0] eq 'DATE-OBS=') then out = 'found' else $
                  cntr = cntr + 1
;******************************************** REMOVE **************************
                if (cntr gt n_elements(header)-1) then begin
                    print,'Bad Frame!!!!  ',trim[k]
;                    pause
                    time[k] = time[k-1]+0.1
                    goto,jump0
                endif
...
;********************************** THE FRATIO CODE IS RUN HERE **********************************
            ...
            FRtemp = fratioF(fiber,cstepsize,psize,winsize,radius=starting_radius,rstep=radstep,EE=eenergy)
