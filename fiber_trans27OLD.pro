PRO fiber_trans2.7, fitslist, APPTYPE=apptype, DARKSWITCH=darkswitch
...
pauseplot = 'on' ;turn this to 'on' and the plots will be paused rather than just a delay
...
;flatCir = readfits('/work1/jdm/fibers/re-imager/flatfield/built_flat.fits',/silent)
;flatCir = float(flatCir)
...
;average radial spacing between all the fibers in the image.
   ...
   sd = sqrt(m[1])
   m = [m,sd]
   xreal = 340.0/fiberarray[0,*] ;the x-position, converted to microns
   yreal = 340.0/fiberarray[1,*] ;the y-position, converted to microns
   rreal = 340.0/fiberarray[5,*] ;the radial spacing between fibers, in microns

   if (plottingscale eq 'on') then begin
      window,0,retain=2
      plot,radius,/ynozero,ytitle='Fiber Spacing (pixels)'
...
; the data is written out to a text file
   ...
   printf,5,strn(m)
   printf,5,''
   form0 = '(a5,4x,a5,6x,a5,6x,a5)'
   printf,5,['Fib #','X_pos','Y_pos','Rad'],format=form0
   form1 = '(i3,5x,f9.4,2x,f9.4,2x,f9.4)'
   for k=0,nf-1 do begin
      if (counter[all] eq 0) then piece = [k+1,xreal,yreal,rreal]
      if (counter[all] eq 1) then piece = [k+nf+1,xreal,yreal,rreal]
