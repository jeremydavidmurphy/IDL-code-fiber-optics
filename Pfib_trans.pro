...
symbols = [1,2,4,5,6,7,1,2,4,5,6,7,1,2,4,5,6,7]
...
;    data[*,j,2] = [a[7],b[7],c[7],d[7],e[7],f[7],g[7],h[7],i[7]]
    data[*,j,0] = [0,0,c[5],d[5],e[5],f[5],g[5],h[5],i[5]]
    data[*,j,1] = [0,0,c[6],d[6],e[6],f[6],g[6],h[6],i[6]]
    data[*,j,2] = [0,0,c[7],d[7],e[7],f[7],g[7],h[7],i[7]]
...
plot,wave,data[*,0,0],xrange=[360,max(wave)+20],$
...
loadct,nct
for j=1,n0-1 do xyouts,0.8,0.5-(j*0.03),names[j],$
  color=colors[j-1],/normal
...
if (ans eq 'y') then begin
    ...
    plot,wave,dataout[*,0],psym=1,title=titl,$
      ...
      xthick=3,ythick=3,charthick=3,thick=2,/nodata
    loadct,4
    for j=0,n0-1 do begin
        if (j eq 4) then begin
            w = [350,365,440,480,520,560,600]
            d = [data[0:1,j,2],data[4:8,j,2]]
            oplot,w,d,psym=symbols[j],color=colors[j],thick=2
            goto, jump1
        endif
        oplot,wave,dataout[*,j],psym=symbols[j],color=colors[j],thick=2
        jump1:
