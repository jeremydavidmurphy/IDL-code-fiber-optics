PRO fiber_transfull, fitslist, APPTYPE=apptype, DARKSWITCH=darkswitch
  
; NOTE: This is just a reworking of fiber_trans4 to expect a single
; frame rather than 2 frames that the code stitches together. I have
; cut out most of the notes (they remain the same as from
; fiber_trans4) for simplicity and to highlight how this code runs.

; the fitslist has the following form...
; EX:
; refframe.fits  dark.fits  deadfibers1t.txt
...
for all = 0,nframes-1 do begin    ;a loop through each individual fits frame
   ...
   image = readfits(fitsfile[all],/silent)
   image = float(image)
